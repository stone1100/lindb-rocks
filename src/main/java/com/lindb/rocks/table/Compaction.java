...
public class Compaction {
    ...
    // all L >= level_ + 2).
    private final int[] levelPointers = new int[DBConstants.NUM_LEVELS];
    ...
    public boolean isTrivialMove() {
        ...
        return (levelInputs.size() == 1 &&
                ...
                totalFileSize(grandparents) <= VersionSet.MAX_GRAND_PARENT_OVERLAP_BYTES);
    ...
    public boolean isBaseLevelForKey(byte[] userKey) {
        ...
        for (int level = this.level + 2; level < DBConstants.NUM_LEVELS; level++) {
    ...
    public boolean shouldStopBefore(InternalKey internalKey) {
        ...
        if (overlappedBytes > VersionSet.MAX_GRAND_PARENT_OVERLAP_BYTES) {
